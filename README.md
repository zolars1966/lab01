...
## 12. Двойное подключение

Возьмем модули из задачи 4. Добавим в `main.cpp` строчку повторного включения `goodbye.h`.
При компиляции выходит ошибка: переопределение существующей функции.
Добавим в `goodbye.cpp` защиту от переопределения:
...
Теперь при компиляции переопределения не происходит и программа работает верно.
